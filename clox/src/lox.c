...
static inline void *reallocate(
    ctx_t const *ctx, vm_t *vm, void const *ptr, usize old_size, usize new_size)
{
    (void)vm;
    if (new_size) {
#if DEBUG_STRESS_GC
...
#define FREE(ptr_) reallocate(ctx, vm, ptr_, 0, 0)
...
#define FREE_ARRAY(type_, ptr_, cap_) \
    reallocate(ctx, vm, (ptr_), sizeof(type_) * (cap_), 0)
    ...
    ctx_t const *ctx, vm_t *vm, usize sz, obj_type_t ot)
{
    ...
    GC_LOGF("%p allocate %U for %d", obj, sz, ot);
...
{
    GC_LOGF("%p free type %d", obj, obj->type);
...
static void mark_roots(ctx_t const *ctx, vm_t *vm)
{
    ...
    mark_compiler_roots(ctx, vm, vm->compiler);
}
...
static void mark_compiler_roots(
    ctx_t const *ctx, vm_t *vm, compiler_t *compiler)
{
    for (compiler_t *c = compiler; c; c = c->enclosing)
        mark_obj(ctx, vm, (obj_t *)c->compiling_func);
}
    ...
    ctx_t const *ctx, vm_t *vm, compiler_t *compiler)
{
    ...
    free_upvalue_array(ctx, vm, &compiler->upvalues);
